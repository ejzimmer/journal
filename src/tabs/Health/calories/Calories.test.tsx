...
describe('Calories', () => {
  ...
  describe('editing an existing day', () => {
    it("opens the form pre-populated with that day's values when its dot is clicked, and updates it on submit", async () => {
      ...
      expect(
        screen.getByRole('heading', { name: '3 Jan' }),
      ).toBeInTheDocument();
    ...
    it("switches to the newly clicked day's form when another dot is clicked while a form is already open", async () => {
      ...
      expect(
        screen.getByRole('heading', { name: '2 Jan' }),
      ).toBeInTheDocument();
      ...
      expect(
        screen.getByRole('heading', { name: '4 Jan' }),
      ).toBeInTheDocument();
      expect(
  ...
  describe('period tracking', () => {
    ...
    it('shows a checkbox for each tracker in the calorie form, checked to match existing trackers', async () => {
      ...
      expect(
        screen.getByRole('checkbox', { name: 'light flow' }),
      ).not.toBeChecked();
      expect(
        screen.getByRole('checkbox', { name: 'heavy flow' }),
      ).toBeChecked();
      expect(
        screen.getByRole('checkbox', { name: 'ovary pain' }),
      ).not.toBeChecked();
    });
